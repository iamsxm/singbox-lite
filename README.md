...
- 🎯 **自定义名称** - 所有节点支持自定义命名
- 🛡️ **入站 IP 白名单** - 新建节点可限制允许连接的来源 IPv4/IPv6 地址或 CIDR
...
- 🛡️ **入站 IP 白名单** - 支持按来源 IPv4/IPv6 地址或 CIDR 限制节点访问
...
添加 sing-box 或 Xray 节点时，向导会询问“允许的入站来源 IP/CIDR”：

```
203.0.113.10, 2001:db8::/32
```

多个地址可用逗号或空格分隔；直接回车表示不限制。脚本会对该节点生成“允许列表 + 其余来源拒绝”的路由规则，并在查看节点时显示当前白名单。白名单限制的是代理服务看到的连接来源；如果节点经过 Cloudflare/CDN，服务端通常看到的是 CDN 回源地址。
